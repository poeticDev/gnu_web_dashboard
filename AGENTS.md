...
## 프로젝트 구조 및 모듈 구성
- 핵심 소스는 `lib/`에 있으며 기능 영역(`camera/`, `media/`, `message/`, `timetable/`, `state/`)으로 분리한다.
- 공용 위젯·테마·유틸은 `lib/common/`에 두고, 진입점은 `lib/main.dart`에 유지한다.
- 상태 관리 레이어는 `lib/state/` 아래 컨트롤러·뷰·유틸로 나뉘며, 페이지 조립은 `state_view.dart`에서 담당한다.
- 테스트는 `test/`에 기능 단위로 미러링하고, 정적 자산은 `assets/test/`, 환경 설정은 `assets/env/`, 웹 셸은 `web/`에 둔다.
...
## 커밋 및 PR 가이드
- Git 히스토리는 `Feat:`, `Fix:` 중심의 접두사를 사용하므로 동일한 스타일을 유지한다(예: `Fix: media 초기화 순서 조정`).
...
## 아키텍처 원칙
- Riverpod 컨트롤러와 `get_it`을 통해 의존성을 주입하며, 서비스·리포지토리는 싱글톤, 컨트롤러는 레이지 싱글톤으로 등록한다.
